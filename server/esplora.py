...
def address_info(address):
    ...
    if data["error"] is None:
        ...
        return Response(
            "Invalid Bitcoin address", mimetype="text/plain", status=400
        )
...
def block_status(bhash):
    ...
    if data["error"] is None:
        result = data["result"]
        next_best = (
            result["nextblockhash"] if "nextblockhash" in result else None
        )
...
def transaction_info(thash):
    ...
    if data["error"] is None:
        ...
        return Response(
            "Transaction not found", mimetype="text/plain", status=404
        )
...
@stats.rest
@blueprint.route(
    "/address/<string:address>/txs", defaults={"thash": None}, methods=["GET"]
)
@blueprint.route(
    "/address/<string:address>/txs/chain/<string:thash>", methods=["GET"]
)
